...
PLIST* ServerLocator::setCMNode(clustermap::enodetype type, clustermap::CMISNode* node, bool isTemp) {
    PLIST* tempnode = _node[type];
    _node[type] = new PLIST(node);
    _node[type]->temp = isTemp;

    _node[type]->next = tempnode;
    if (tempnode)
        _node[type]->count = tempnode->count + 1;
    else
        _node[type]->count = 1;
}

PLIST* ServerLocator::getNodes(clustermap::enodetype type) {
    return _node[type];
}

void ServerLocator::freeTempNodes(clustermap::enodetype type) {
    PLIST* piter = _node[type];
    while (piter != NULL) {
        if (piter->temp) {
            if (piter == _node[type])
                _node[type] = piter->next;
            PLIST* pnext = piter->next;
            delete piter;
            piter = pnext;
        }
        else 
