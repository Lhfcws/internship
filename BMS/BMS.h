...
static const char* BLENDER_CFG="conf/blender_server.cfg";
static const char* SEARCHER_CFG="conf/searcher_server.cfg";
static const char* DETAILER_CFG="conf/detail_server.cfg";
...
struct PLIST {
    ...
    PLIST():count(1), next(NULL), temp(false) {
        node = new clustermap::CMISNode();
    }

    PLIST(clustermap::CMISNode* node_):count(1), next(NULL), temp(false) {
        node = node_;
    }
